...
Let $\Omega \subset \mathbb R^2$ and consider the problem
\begin{eqnarray}\label{eq:pde}
    ...
    \end{aligned}
\end{eqnarray}
Equation~\eqref{eq:pde} defines a mapping $\mathcal S: L^2(\Omega) \rightarrow L^2(\Omega)$. If $f \in L^2$ is the source function (the input), then Equation~\eqref{eq:pde} maps $f$ to $\mathcal S f = u$ (the output). Introduce the trace operator $\mathcal T: L^2(\Omega) \rightarrow L^2(\partial\Omega)$. This is the observed values of $u$ on the boundary of $\Omega$,
\begin{equation}\label{eq:trace}
    \mathcal T: u \mapsto y = u|_{\partial\Omega}.
\end{equation}
Operator~\eqref{eq:pde} and \eqref{eq:trace} defines the forward operator $\mathcal K: L^2(\Omega) \rightarrow L^2(\partial\Omega)$, 
\begin{equation}
    \mathcal K = \mathcal T \circ \mathcal S,
\end{equation}
which is a mapping from the source function $f$ to the observed output $y = \mathcal K f$.

The problem which I investigate in this Master's thesis is: given observed boundary values $y = u|_{\partial\Omega}$, find a source $f$ such that $y = \mathcal K$. This is visualized in Fig.~\ref{fig:forward_operator}.


\subsection{The finite element method}
Let $\mathcal M$ be a mesh of $\Omega$ and define the finite element space

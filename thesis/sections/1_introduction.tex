...
In this thesis, we study the class of elliptic PDEs, which typically occur in time-independent problems. This class of PDEs show up in steady-state problems, stochastic processes, electrostatics, and the diffusion and flow a quantity in a region of space \cite{pavliotisStochasticProcessesApplications2014, evansPartialDifferentialEquations2010,straussPartialDifferentialEquations2007}.
...
This address this issue, we propose a low-rank approximation strategy based on the randomized singular value decomposition (rSVD). The key idea is to use random sampling of $\mathcal{K}$ to find an approximate rank-$k$ SVD $K_k = U \Sigma V^T$, which then can be used to effectively construct the weights $W$. Due to the large null space of $\mathcal{K}$, and it's rapid decay of singular values, the target rank $k$ can be selected much smaller than $N$.

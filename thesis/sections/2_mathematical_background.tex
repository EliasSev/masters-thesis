...
Standard Tikhonov ($W = I$) does not work here. Because $K$ has a large null space, the solutions it produces concentrate near the domain boundary regardless of where the true source is located \cite{elvetunRegularizationOperatorSource2020}. This illustrated in Figure~\ref{fig:standard_tikhonov_solution}.
...
\end{equation*}
where $\hat P: V_h \to \operatorname{Nul}(K)^\perp$ is the orthogonal projection onto $\operatorname{Nul}(K)^\perp$. If $\| \hat P \phi_i \|$ is small, then $\langle \hat Pf, \phi_i \rangle$ is small, and the corresponding component of $f^*$ is small. This means that for any $\phi_i$ far away from $\partial \Omega$, the corresponding component of $f^*$ gets small, introducing bias to the solution. This issue transfers to the standard Tikhonov solution $f_\lambda$ since $f_\lambda \to f^*$ as $\lambda \to 0$.

Elvetun and Nielsen counteracts this by re-scaling the norm of $\hat P \phi_i$ such that $\| \hat P \phi_i \| = 1$. This is achieved by setting $w_i = \| \hat P \phi_i \|$ with $W = \operatorname{diag}(w_i)$ in the Tikhonov formulation, equalizing the contribution of all components. In the discrete formulation, $\hat P = K^+ K$ is the orthogonal projection onto the complement of the null space, and the weights are computed as $w_i = \|K^+ K e_i\|$.

In our setting, the basis of the element space $V_h$ given in Equation~\eqref{eq:fem_space} needs to be normalized. If $K = U\Sigma V^T$ is the SVD of $K$ and $M_{ii}$ is the $i$'th diagonal element of the mass matrix $M$, then the weights are computed as 

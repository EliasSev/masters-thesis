...
\begin{equation}
    ...
    \text{ for all } v \in H^1(\Omega).
\end{equation}
The solution $w$ then satisfies $\mathcal K^* g = w$. For details, see 
...
Replacing the continuous operator $\mathcal{K}$ by its discretization $K \in \mathbb{R}^{N_b \times N}$ and the $L^2$ norms by the FEM inner products defined in Equations~\eqref{eq:fem_norm} and \eqref{eq:fem_norm_ds}, the inverse problem \eqref{eq:problem} takes the discrete form
...
\end{equation}
where $\mathbf{x} \in \mathbb{R}^N$ is the coefficient vector of the source
$f_h \in V_h$, $\mathbf{y} \in \mathbb{R}^{N_b}$ is the vector of observed boundary data, $\lambda \geq 0$ is the regularization parameter, and $W \in \mathbb{R}^{N \times N}$ is a regularization weight matrix to be specified.

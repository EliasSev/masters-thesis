...
\subsection{A weighted formulation}
Assume that $x^*$ solves the linear system $A x = y$ where $A \in \mathbb{R}^{m \times n}$. Then, clearly, $x^*$ solves $B A x = B y$ for a matrix $B \in \mathbb{R}^{q \times m}$. This motivates modifying the Tikhonov formulation in Equation~\eqref{eq:tikhonov},
\begin{equation*}
    \min_{x \in \mathbb{R}^n} \| B A x - B y \|^2 + \lambda^2 \| W x \|^2.
\end{equation*}

We are interested in finding a $B$ which improves reconstructions of the Tikhonov problem. This is what Elvetun and Nielsen analyzed in a series of paper, in particular, if the choice $B = A^+$ improves inverse solutions. In the setting of the discrete inverse problem in Equation~\eqref{eq:discrete_inverse_problem}, they analyze the problem
\begin{equation}\label{eq:transformed_discrete_inverse_problem}
    \min_{x \in \mathbb{R}^N}
    \| K^+ K x - K^+ y \|_{M}^2
    + \lambda^2 \| W x \|_M^2.
\end{equation}
In this instance, $K^+ K = \hat P$, which is the orthogonal projection onto the complement of the null space $\operatorname{Nul}(K)$ of $K$.

\textcolor{red}{TODO: This subsection is not done}
...
\begin{equation*}
    A = L U,
\end{equation*}
where $L$ is a lower triangular matrix and $U$ an upper triangular matrix. $A$ has an LU decomposition under the condition that $\operatorname{det}(A[1:k, 1:k]) \neq 0$ for $k=1,\dots, n-1$. More generally, any square matrix $A$ has a LU decomposition with partial pivoting (LUP), $PA = LU$. Computing the LU decomposition costs $\mathcal{O}(n^3)$ flops, specifically, $\tfrac{2}{3}n^3$.

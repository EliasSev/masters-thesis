...
\paragraph{Sobolev space.} The space $H^1(\Omega)$, known as the first-order Sobolev space, is defined as
\begin{equation*}
    H^1(\Omega) =
    \left\{
        f \in L^2(\Omega) \, \Bigg| \,
...
\begin{equation*}
    x^T A x > 0
\end{equation*}
for all $x \in \mathbb{R}^n$, then $A$ is said to be symmetric positive definite. For details, see Section~4.2 in \cite{golubMatrixComputations4th2013}.
...
\paragraph{Sparse linear systems.} For sparse systems $A x = y$, computing the $LU$ factorization comes with additional challenges. Matrix entries which are zero in $A$ but non-zero in $L$ or $U$, so called fill-ins, require more memory and result in a more expensive factorization \cite{larsonFiniteElementMethod2013}. Additionally, the cost of solving the resulting triangular systems is reduced. Much research has gone into the development algorithms which avoid fill-ins, for instance, see Davis \cite{davisDirectMethodsSparse2006}.

...
For the problem considered in this thesis, the operator $K$ has a large nullspace. Standard Tikhonov regularization with $W = I$ often leads to solutions which are concentrated  near the boundary of the domain, regardless of the position of the true source, see \cite{elvetun_regularization_2020}.
...
\begin{equation}
    w_i = \|K^+ K e_i\|,
\end{equation}
where $e_i$ is the $i$th standard basis vector of $\mathbb{R}^n$ \cite{elvetun_regularization_2020}. The regularization matrix is then chosen as the diagonal matrix
\begin{equation}
